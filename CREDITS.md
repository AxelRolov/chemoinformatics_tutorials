...
- **T001** Compound data acquisition (ChEMBL) → session 03 (ChEMBL workflow, IC50/pIC50 theory). The cached dataset
  `data/EGFR_compounds_chembl.csv` is the output of this talktorial.
- **T002** Molecular filtering: ADME and lead-likeness → session 04 (property filters, Ro5 statistics).
- **T003** Unwanted substructures → session 04 (PAINS).
...
- `misc/ChEMBL_data_curation`, `working_with_ChEMBL_drug_data` → sessions 03–04.
- `clustering/taylor_butina_clustering`, `kmeans_clustering` → session 04.
- `sar_analysis/find_scaffolds`, `R_group_analysis` → session 04 (scaffold analysis).
- `misc/visualizing_chemical_space` → session 04 (PCA + t-SNE/UMAP workflow).
- `ml_models/*` (`regression_model`, `classification_model`, `cross_validation`, `comparing_*`, `QSAR_in_8_lines`) → session 05.
- `generative/SMILES_RNN` → session 07.
- `chemprop/run_chemprop` → session 06.
- Datasets: `data/hERG.csv` (session 04 exercises) and `chembl_drugs.smi` from
...
- `02 - Supervised Learning` → session 05.
- `03 - Intro to Deep Learning` (`01_intro_to_dl`, `02_graph_nns`, `03_gnn_simple_example`) → session 06.
- `04 - Unsupervised Learning` (`Clustering`, `DimensionalityReduction`) → session 04.
- `05 - Generative Models`, `06 - Generative Models 2 / SMILES-LSTM-Walkthrough` → session 07.
...
- `06_sklearn_fitting_1`, `07_ESOL_fitting` → session 05.
...
<https://github.com/huggingface/smolagents> · **Apache-2.0** — the agent framework used in session 08
...
session 06 (first neural net → PyTorch → GNN). No code reproduced (note the NonCommercial-ShareAlike terms).
...
| [scikit-learn](https://scikit-learn.org) | classical ML (session 05) | BSD-3 |
| [XGBoost](https://xgboost.readthedocs.io) | gradient boosting (session 05) | Apache-2.0 |
| [PyTorch](https://pytorch.org) / [PyTorch Geometric](https://pytorch-geometric.readthedocs.io) | deep learning, GNNs (sessions 06–07) | BSD-3 / MIT |
| [UMAP](https://github.com/lmcinnes/umap) | chemical-space maps (session 04) | BSD-3 |
...
| [chembl_webresource_client](https://github.com/chembl/chembl_webresource_client) | ChEMBL API (session 03) | Apache-2.0 |
...
| [CReM](https://github.com/DrrDom/crem) (P. Polishchuk) | mutation-based design idea (session 07) | BSD-3 |
| [GuacaMol](https://github.com/BenevolentAI/guacamol) / [MOSES](https://github.com/molecularsets/moses) | generative metrics (session 07) | MIT |
| [OpenADMET](https://github.com/OpenADMET) challenge tutorials & datasets | ADMET data (session 03) | Apache-2.0; ExpansionRx data CC BY 4.0 |
