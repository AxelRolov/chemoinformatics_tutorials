...
| `EGFR_curated.csv` | 5511 | the same set after standardisation, tautomer canonicalisation, InChIKey deduplication, median aggregation and removal of contradictory replicates; plus `active` (pIC50 ≥ 6.3) and Murcko `scaffold` | produced by `notebooks/05_exploratory_data_analysis.ipynb` | data ChEMBL CC BY-SA 3.0 |
...
```bash
# EGFR_curated.csv - run the session 05 notebook, or the equivalent short script:
python - <<'PY'
import pandas as pd
from rdkit import Chem
from rdkit.Chem.MolStandardize import rdMolStandardize
from rdkit.Chem.Scaffolds import MurckoScaffold
# ... see notebooks/05_exploratory_data_analysis.ipynb, sections 1 and 4
PY
